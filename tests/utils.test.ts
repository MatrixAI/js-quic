...
describe('utils', () => {
  ...
  test('to and from IPv4 mapped IPv6 addresses', () => {
    ...
    expect(utils.toIPv4MappedIPv6('255.255.255.255')).toBe('::ffff:255.255.255.255');
    ...
    expect(utils.fromIPv4MappedIPv6('::ffff:255.255.255.255')).toBe('255.255.255.255');
    expect(utils.fromIPv4MappedIPv6('::ffff:ffff:ffff')).toBe('255.255.255.255');
    ...
    expect(utils.toIPv4MappedIPv6(utils.fromIPv4MappedIPv6('::ffff:7f00:1'))).toBe('::ffff:127.0.0.1');
  });
  test('resolves zero IP to local IP', () => {
    ...
    expect(utils.resolvesZeroIP('::ffff:0.0.0.0' as Host)).toBe('::ffff:127.0.0.1');
