...
import QUICConnectionClient from './src/QUICConnectionClient';
...
async function main() {
  ...
  const connClient = await QUICConnectionClient.connectQUICConnection({
    scid,
    socket,
    remoteInfo: {
      host: '127.0.0.1' as Host,
    ...
    config: clientConfig,
    logger: logger.getChild('--CLIENT CONNECTION--')
  });

  await testsUtils.sleep(1000);

  console.log(connClient.conn.timeout());


  await connClient.destroy();
  await socket.stop();
