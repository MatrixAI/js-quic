...
pub enum PathEvent {
    New {
        local: connection::HostPort,
        peer: connection::HostPort,
    },
    Validated {
        local: connection::HostPort,
        peer: connection::HostPort,
        ...
        match path_event {
            ...
            quiche::PathEvent::ReusedSourceConnectionId(seq, old, new) => {
                PathEvent::ReusedSourceConnectionId {
                    seq,
                    old: (
                        connection::HostPort::from(old.0),
                        connection::HostPort::from(old.1),
                    ),
                    new: (
                        connection::HostPort::from(new.0),
                        connection::HostPort::from(new.1),
                    ),
                }
            }
...
impl Generator for HostIter {
    ...
    fn next(&mut self, _value: Option<Self::Next>) -> Option<Self::Yield> {
        return self.0.next().map(|socket_addr| socket_addr.into());
    }
}
...
impl Generator for PathStatsIter {
    ...
    fn next(&mut self, _value: Option<Self::Next>) -> Option<Self::Yield> {
        return self.0.next().map(|path_stats| path_stats.into());
    }
}
