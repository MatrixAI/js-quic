...
impl Config {
    ...
    pub fn with_boring_ssl_ctx(
        ...
        if let Some(ca) = ca {
            let mut x509_store_builder = boring::x509::store::X509StoreBuilder::new()
                .or_else(|e| Err(napi::Error::from_reason(e.to_string())))?;
            let x509_certs = boring::x509::X509::stack_from_pem(&ca.to_vec())
                .or_else(|e| Err(napi::Error::from_reason(e.to_string())))?;
            ...
            ssl_ctx_builder
                .set_verify_cert_store(x509_store)
                .or_else(|e| Err(napi::Error::from_reason(e.to_string())))?;
        }
        ...
        if let (Some(key), Some(cert)) = (key, cert) {
            ...
            ssl_ctx_builder
                ...
                .or_else(|e| Err(napi::Error::from_reason(e.to_string())))?;
            let x509_cert_chain = boring::x509::X509::stack_from_pem(&c)
                .or_else(|err| Err(napi::Error::from_reason(err.to_string())))?;
        ...
        let config =
            quiche::Config::with_boring_ssl_ctx_builder(quiche::PROTOCOL_VERSION, ssl_ctx_builder)
                .or_else(|e| Err(Error::from_reason(e.to_string())))?;
        return Ok(Config(config));
    }
    ...
    pub fn load_cert_chain_from_pem_file(&mut self, file: String) -> Result<()> {
        return self
            .0
            .load_cert_chain_from_pem_file(&file)
            .or_else(|err| Err(Error::from_reason(err.to_string())));
    }
    ...
    pub fn load_priv_key_from_pem_file(&mut self, file: String) -> Result<()> {
        return self
            .0
            .load_priv_key_from_pem_file(&file)
            .or_else(|err| Err(Error::from_reason(err.to_string())));
    }
    ...
    #[napi]
    pub fn set_application_protos(&mut self, protos_list: Vec<String>) -> Result<()> {
        let protos_list = protos_list
            .iter()
            .map(|proto| proto.as_bytes())
            .collect::<Vec<&[u8]>>();
        return self
            .0
            .set_application_protos(&protos_list)
            .or_else(|err| Err(Error::from_reason(err.to_string())));
    }

    #[napi]
    pub fn set_application_protos_wire_format(&mut self, protos: Uint8Array) -> Result<()> {
        return self
            .0
            .set_application_protos_wire_format(&protos)
            .or_else(|err| Err(Error::from_reason(err.to_string())));
    }
    ...
    pub fn set_max_recv_udp_payload_size(&mut self, size: i64) -> () {
        return self.0.set_max_recv_udp_payload_size(size as usize);
    }

    #[napi]
    pub fn set_max_send_udp_payload_size(&mut self, size: i64) -> () {
        return self.0.set_max_send_udp_payload_size(size as usize);
    }
    ...
    #[napi]
    pub fn set_initial_max_streams_uni(&mut self, v: i64) -> () {
        return self.0.set_initial_max_streams_uni(v as u64);
    }

    #[napi]
    pub fn set_ack_delay_exponent(&mut self, v: i64) -> () {
        return self.0.set_ack_delay_exponent(v as u64);
    }

    #[napi]
    pub fn set_max_ack_delay(&mut self, v: i64) -> () {
        return self.0.set_max_ack_delay(v as u64);
    }

    #[napi]
    pub fn set_active_connection_id_limit(&mut self, v: i64) -> () {
    ...
    #[napi]
    pub fn enable_dgram(&mut self, enabled: bool, recv_queue_len: i64, send_queue_len: i64) -> () {
        return self
            .0
            .enable_dgram(enabled, recv_queue_len as usize, send_queue_len as usize);
    }
    ...
    pub fn set_stateless_reset_token(&mut self, v: Option<BigInt>) -> () {
        return self.0.set_stateless_reset_token(v.map(|v| v.get_u128().1));
    }
