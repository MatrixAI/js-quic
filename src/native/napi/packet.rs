...
impl Header {
    ...
    pub fn from_slice(mut data: Uint8Array, dcid_len: i64) -> napi::Result<Self> {
        return quiche::Header::from_slice(&mut data, dcid_len as usize)
...
pub fn negotiate_version(
    ...
    return quiche::negotiate_version(&scid, &dcid, &mut data)
        .or_else(|e| Err(Error::from_reason(e.to_string())))
        .map(|v| v as i64);
}
...
pub fn retry(
    ...
    return quiche::retry(&scid, &dcid, &new_scid, &token, version, &mut out)
        .or_else(|e| Err(Error::from_reason(e.to_string())))
        .map(|v| v as i64);
}
