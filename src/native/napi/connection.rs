...
impl From<quiche::Stats> for Stats {
    fn from(stats: quiche::Stats) -> Self {
        return Stats {
            ...
            peer_initial_max_stream_data_bidi_local: stats.peer_initial_max_stream_data_bidi_local
                as i64,
            peer_initial_max_stream_data_bidi_remote: stats.peer_initial_max_stream_data_bidi_remote
                as i64,
...
impl Connection {
    ...
    pub fn connect(
        ...
        let local_addr: SocketAddr = local_host
            .try_into()
            .or_else(|err: io::Error| Err(napi::Error::from_reason(err.to_string())))?;
        let remote_addr: SocketAddr = remote_host
            .try_into()
            .or_else(|err: io::Error| Err(napi::Error::from_reason(err.to_string())))?;
    ...
    pub fn accept(
        ...
        let remote_addr: SocketAddr = remote_host
            ...
            .or_else(|err: io::Error| Err(napi::Error::from_reason(err.to_string())))?;
        let scid = quiche::ConnectionId::from_ref(&scid);
        let odcid = odcid.map(|dcid| quiche::ConnectionId::from_vec(dcid.to_vec()));
    ...
    #[napi]
    pub fn recv(&mut self, mut data: Uint8Array, recv_info: RecvInfo) -> napi::Result<i64> {
    ...
    pub fn send_on_path(
        ...
        let from: Option<SocketAddr> = match from {
            Some(host) => Some(host.try_into().or_else(|err: io::Error| {
    ...
    pub fn send_quantum_on_path(
        ...
        let local_addr: SocketAddr = local_host.try_into().or_else(|err: io::Error| {
            Err(napi::Error::new(napi::Status::InvalidArg, err.to_string()))
        })?;
    ...
    #[napi]
    pub fn probe_path(&mut self, local_host: HostPort, peer_host: HostPort) -> napi::Result<i64> {
        let local_addr: SocketAddr = local_host.try_into().or_else(|err: io::Error| {
            Err(napi::Error::new(napi::Status::InvalidArg, err.to_string()))
        })?;
        let peer_addr: SocketAddr = peer_host.try_into().or_else(|err: io::Error| {
            Err(napi::Error::new(napi::Status::InvalidArg, err.to_string()))
        })?;
        return self
            .conn
    ...
    #[napi]
    pub fn is_path_validated(&self, from: HostPort, to: HostPort) -> napi::Result<bool> {
        let from_addr: SocketAddr = from.try_into().or_else(|err: io::Error| {
            Err(napi::Error::new(napi::Status::InvalidArg, err.to_string()))
        })?;
        let to_addr: SocketAddr = to.try_into().or_else(|err: io::Error| {
            Err(napi::Error::new(napi::Status::InvalidArg, err.to_string()))
        })?;
        return self
            .conn
